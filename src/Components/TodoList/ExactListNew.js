...
function CheckboxList(props) {
  ...
  const handleDelete=(id) => {
    ...
    const onChange1=(e,id) => {
      ...
      props.setArrayy([...props.arrayy.slice(0,id),{...elem,text:e.target.value},...props.arrayy.slice(id+1)])
      }
  ...
  const handleUpdate=(id) =>{
    const elem = props.arrayy[id]
    props.setArrayy([...props.arrayy.slice(0,id),{...elem,editMode:false},...props.arrayy.slice(id+1)])
    ...
    <List className={classes.root}>
      {props.arrayy.map((elem,index) => (
        <ListItem key={elem} role={undefined} dense button onClick={()=>handleToggle(elem)}>
          ...
          { elem.editMode===false?    <ListItemText className="listText"  primary={elem.text} onClick={() => handleEdit(index)} id={elem.key} />
          :<div className="divEdit">
             <TextField id="standard-basic" label="" value={elem.text} onChange={(e)=>onChange1(e, index)}/>
             {/* <EditText/> */}
             <IconButton aria-label="Comments" onClick={() => handleUpdate(elem.key)} id={elem.key}>
              <DoneIcon/>
            </IconButton>   
